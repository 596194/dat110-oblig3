...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		//Node findsucc = (Node) node.findSuccessor(key);
		...
		if(Util.checkInterval(key, node.getNodeID().add(new BigInteger("1")), succ.getNodeID())) {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for(int i = fingertable.size()-1; i >= 0 ; i--) {
			BigInteger id = fingertable.get(i).getNodeID();
			if(Util.checkInterval(id, node.getNodeID().add(new BigInteger("1")), ID.subtract(new BigInteger("1")))) {
